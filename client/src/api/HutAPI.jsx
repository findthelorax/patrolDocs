...
export const api = {
	async createHut(mountainId, hut) {
		try {
			const response = await axios.post(`${IP}:${PORT}/mountain/${mountainId}/hut`, hut);
			return response.data;
		} catch (error) {
			console.error(`Error creating hut for mountain with id ${mountainId}`, error);
			throw error;
		}
	},
    ...
    async createHutLog(mountainId, hutId, log) {
		try {
			...
			console.error(`Error creating log for hut with id ${hutId} for mountain with id ${mountainId}`, error);
			throw error;
		}
	},
	...
	async getHutLog(mountainId, hutId, logId) {
		try {
			...
			console.error(
				`Error fetching log with id ${logId} for hut with id ${hutId} for mountain with id ${mountainId}`,
				error
			);
			throw error;
		}
	...
	async updateHutLog(mountainId, hutId, logId, updatedLog) {
		try {
			const response = await axios.put(
				`${IP}:${PORT}/mountain/${mountainId}/hut/${hutId}/log/${logId}`,
				updatedLog
			);
			return response.data;
		} catch (error) {
			console.error(
				`Error updating log with id ${logId} for hut with id ${hutId} for mountain with id ${mountainId}`,
				error
			);
			throw error;
		}
	...
	async deleteHutLog(mountainId, hutId, logId) {
		try {
			...
			console.error(
				`Error deleting log with id ${logId} for hut with id ${hutId} for mountain with id ${mountainId}`,
				error
			);
			throw error;
		}
